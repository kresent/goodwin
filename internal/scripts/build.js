...
let productionPrefix = args.find(arg => arg.includes('--production_prefix'));
productionPrefix = productionPrefix
  ? `${productionPrefix.split('=')[1]}`
  : null;
...
Object.keys(config('bundles'))
  ...
  .forEach(bundleName => {
    const compiler = webpack(
      webpackConfigFactory({ target: bundleName, optimize, productionPrefix })
    );
