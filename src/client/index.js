...
setTimeout(() => injectTapEventPlugin(), 0);

import './polyfills';

import ReactHotLoader from './components/ReactHotLoader';
import App from '../shared/root/App';
...
function renderApp(TheApp) {
  ...
  const Router = process.env.CORDOVA ? HashRouter : BrowserRouter;
      ...
      <AsyncComponentProvider rehydrateState={asyncComponentsRehydrateState}>
        <JobProvider rehydrateState={rehydrateState}>
          <Provider store={store}>
            <Router
              basename={process.env.PRODUCTION_PREFIX}
...
if (process.env.BUILD_FLAG_IS_DEV === 'true' && module.hot) {
  ...
  module.hot.accept('../shared/root/App', () => {
    renderApp(require('../shared/root/App').default);
  });
}
